...
func NewClient(credentials *Credentials) (*Client, error) {
	...
	if credentials.SessionAuth {
		fmt.Printf("[DEBUG] Using session_auth\n")
...
func (c *Client) NewRequest(ctx context.Context, method, urlStr string, body interface{}) (*http.Request, error) {
	return c.NewRequestBasePath(ctx, method, urlStr, body, absolutePath)
	// return req, nil
}

// NewRequest creates a request
func (c *Client) NewRequestBasePath(ctx context.Context, method, urlStr string, body interface{}, basePath string) (*http.Request, error) {
	rel, errp := url.Parse(basePath + urlStr)
	if errp != nil {
		return nil, errp
	}
	...
	if c.Cookies != nil {
		fmt.Printf("[DEBUG] Adding cookies to request\n")
		for _, i := range c.Cookies {
			req.AddCookie(i)
		}
	} else {
		fmt.Printf("[DEBUG] Adding basic auth to request\n")
...
func CheckResponse(r *http.Response) error {
	...
	// if c := r.StatusCode; c > 400 {
	// 	return fmt.Errorf("Bad request (statuscode > 400) error: %s", string(body))
	// }

	buf, err := ioutil.ReadAll(r.Body)

	if c := r.StatusCode; c > 400 {
		return fmt.Errorf("Error occured while performing request: %s", string(buf))
	}
	if err != nil {
		return err
	}
